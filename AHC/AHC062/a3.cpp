...
vector<P> SolveBySA() {
    ...
    RunBandHillClimb(path, currentScore, bestScore, bestPath, twoRowBands, rng);
    path = bestPath;
    ...
    while (true) {
        ...
        TryRandomReverse(path, currentScore, bestScore, bestPath, allTwoRowSegs, temp, false, rng);
    }
    ...
    while (chrono::duration<double>(chrono::steady_clock::now() - hillStart).count() < hillTimeLimit) {
        TryRandomReverse(path, currentScore, bestScore, bestPath, allTwoRowSegs, 1.0, true, rng);
    }
